...
import utilityPackage.kitti_util as utils
import argparse
...
class kitti_object(object):
    ...
    def __init__(self, root_dir, split="training", args=None):
        ...
        self.image_dir_left = os.path.join(self.split_dir, "image_02")
        self.image_dir_right = os.path.join(self.split_dir, "image_03")
    ...
    def get_image(self, idx):
        ...
        imgPath = os.path.join(self.image_dir, "%04d" % (idx))
        img_filename = os.path.join(imgPath, "%06d.png" % (idx))
        return utils.load_image(img_filename)

    def get_image_Left(self, scenceNum, frmeNumber):
        assert scenceNum < self.num_samples
        imgPath = os.path.join(self.image_dir_left, "%04d" % (scenceNum))
        img_filename = os.path.join(imgPath, "%06d.png" % (frmeNumber))
        return utils.load_image(img_filename)

    def get_image_Right(self, scenceNum, frmeNumber):
        assert scenceNum < self.num_samples
        imgPath = os.path.join(self.image_dir_right, "%04d" % (scenceNum))
        img_filename = os.path.join(imgPath, "%06d.png" % (frmeNumber))
        return utils.load_image(img_filename)


    def get_lidar(self, sceneNumber, frame_swipe_number,dtype=np.float32, n_vec=4):
        assert sceneNumber < self.num_samples

        # modifying for Tracking dataset
        lidarPath = os.path.join(self.lidar_dir, "%04d" % (sceneNumber))
        lidar_filename = os.path.join(lidarPath, "%06d.bin" % (frame_swipe_number))
    ...
    def get_frame_Lable_tracks(self,idx, frameNumber):
        assert idx < self.num_samples and self.split == "training"
        label_filename = os.path.join(self.label_dir, "%04d.txt" % (idx))
        objects = utils.read_label(label_filename)
        frameObjects = [obj for obj in objects if obj.frame == frameNumber]
        return frameObjects


    def get_pred_objects(self, idx):
        assert idx < self.num_samples
        pred_filename = os.path.join(self.pred_dir, "%04d.txt" % (idx))
    ...
    def get_depth(self, idx):
        assert idx < self.num_samples
        img_filename = os.path.join(self.depth_dir, "%04d.png" % (idx))
    ...
    def get_depth_pc(self, idx):
        assert idx < self.num_samples
        lidar_filename = os.path.join(self.depthpc_dir, "%04d.bin" % (idx))
    ...
    def isexist_depth(self, idx):
        assert idx < self.num_samples and self.split == "training"
        depth_filename = os.path.join(self.depth_dir, "%04d.txt" % (idx))
...
def show_image_with_boxes(img, objects, calib, show3d=True, depth=None):
    ...
    img1_2d = np.copy(img)  # for 2d bbox
    img2_3d = np.copy(img)  # for 3d bbox
    #img3 = np.copy(img)  # for 3d bbox
    #TODO: change the color of boxes

    # Below code generates the 2D dox on the image based on Labelled objects
    for obj in objects:
        ...
        if(box3d_pts_2d is None):
            ...
            img2_3d = utils.draw_projected_box3d(img2_3d, box3d_pts_2d, color=(0, 255, 0))
        elif obj.type == "Pedestrian":
            img2_3d = utils.draw_projected_box3d(img2_3d, box3d_pts_2d, color=(255, 255, 0))
        elif obj.type == "Cyclist":
            img2_3d = utils.draw_projected_box3d(img2_3d, box3d_pts_2d, color=(0, 255, 255))
        elif obj.type == "Van":
            img2_3d = utils.draw_projected_box3d(img2_3d, box3d_pts_2d, color=(255, 165, 0))
...
def show_lidar_with_depth(
    ...
    from utilityPackage.viz_util import draw_lidar_simple, draw_lidar, draw_gt_boxes3d
    ...
    if depth is not None:
        ...
        if save:
            ...
            save_filename = os.path.join(vely_dir, "%04d.bin" % (data_idx))
            print(save_filename)
