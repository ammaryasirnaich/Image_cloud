...
class tracklet(object):
    def __init__(self):
        ...
        self.frame :int
        self.tracklet_id  :int
        self.type : str  # 'Car', 'Pedestrian', ...
        self.truncation :int # 0=visible, 1=partly occluded, 2=fully occluded, 3=unknown
        self.alpha :float  # object observation angle [-pi..pi]

        # extract 2d bounding box in 0-based coordinates
        self.xmin :float  # left
        self.ymin :float  # top
        self.xmax :float  # right
        self.ymax :float  # bottom
        self.box2d = np.array([self.xmin, self.ymin, self.xmax, self.ymax])

        # extract 3d bounding box information
        self.h : float  # box height
        self.w : float  # box width
        self.l : float  # box length (in meters)
        self.t :tuple #(data[13], data[14], data[15])  # location (x,y,z) in camera coord.
        self.ry :float # data[16]  # yaw angle (around Y-axis in camera coordinates) [-pi..pi]
...
class Object3d(object):
    ...
    def __init__(self, label_file_line,frameNumber = None):
        data = label_file_line.split(" ")

        #Ammar
        #Modifying according to Tracking Truth label dataset
...
def read_label(label_filename):
    check = os.path.isfile(label_filename)
